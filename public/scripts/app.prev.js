  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(position => {
      ...
      window.dispatchEvent(new CustomEvent("loadCameraData", {
        detail: { latne, lngne, latsw, lngsw }
      }));
...
const displayCameraData = (data) => {
  ...
  data.results.forEach(camera => {
    ...
    else { caption.textContent = camera.cameraViews[0].mainRoute; }
    div.appendChild(img);
...
window.addEventListener("loadCameraData", (event) => {
  const { latne, lngne, latsw, lngsw } = event.detail;
   // fetch(`https://us-central1-radarb.cloudfunctions.net/getCameraData?lat=${lat}&lng=${lng}`)
...
// SENSORS
const displaySensorData = (data, userLat, userLng) => {
    ...
    data.results.forEach(result => {
      result.surfaceSensors.forEach(sensor => {
        ...
        const distance = haversine(userLat, userLng, sensor.latitude, sensor.longitude);
